...
NOTE: This module is importable. `run_hourly_job()` is the entrypoint used by
the global scheduler.py. Running this file directly still works standalone
(scheduler.py is not required for manual/dev use).
"""
...
# Logging
# ---------------------------------------------------------------------
# NOTE: when imported by scheduler.py, logging is already configured there
# before this module is imported, so basicConfig() below is a no-op
# (Python only applies basicConfig if the root logger has no handlers yet).
logging.basicConfig(
    level=logging.INFO,
...
# ---------------------------------------------------------------------
# MODULE-LEVEL ENTRYPOINTS (importable by scheduler.py)
# ---------------------------------------------------------------------
# Lazily built so that importing this module doesn't open a ClickHouse
# client connection as a side effect — the client is only constructed
# the first time run_hourly_job() actually fires.
_pipeline_instance: Optional[HourlyAggregationPipeline] = None


def get_pipeline() -> HourlyAggregationPipeline:
    global _pipeline_instance
    if _pipeline_instance is None:
        client = ClickHouseClient(
            CH_HOST, CH_PORT, CH_USERNAME, CH_PASSWORD, CH_DATABASE
        )
        _pipeline_instance = HourlyAggregationPipeline(client)
    return _pipeline_instance


def run_hourly_job():
    """Entrypoint used both standalone (below) and by the global scheduler.py."""
    try:
        get_pipeline().run()
...
# ---------------------------------------------------------------------
# ENTRY POINT (standalone use only — scheduler.py does not execute this)
# ---------------------------------------------------------------------
if __name__ == "__main__":
    # Run immediately on start, then every hour at :30
    run_hourly_job()
    schedule.every().hour.at(":30").do(run_hourly_job)
